...
export default function ManageAdmin({ admins = [], deactivatedAdmins = [] }) {
    ...
    const displayedAdmins = activeTab === 'active' ? admins : deactivatedAdmins;
    ...
    const handleCreate = (e) => {
        e.preventDefault();
        createForm.post(route('admin.manageadmins.store'), {
            onSuccess: () => {
                createForm.reset();
    ...
    const handleEdit = (e) => {
        e.preventDefault();
        editForm.put(route('admin.manageadmins.update', selectedAdmin.id), {
            onSuccess: () => {
                editForm.reset();
    ...
    const handleDeactivate = () => {
        router.patch(route('admin.manageadmins.deactivate', selectedAdmin.id), {}, {
            onSuccess: () => {
    ...
    const openActivateModal = (admin) => {
        setSelectedAdmin(admin);
        setShowActivateModal(true);
    };

    const handleActivate = () => {
        router.patch(route('admin.manageadmins.activate', selectedAdmin.id), {}, {
            onSuccess: () => {
        ...
        <DashboardLayout title="Manage Admin">
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-green-700 dark:text-green-200 text-sm">
                            Create new admin accounts and view all existing admins.
                        </p>
                    </div>
                    <button
                                ...
                                {displayedAdmins.length === 0 ? (
                                    <tr>
                                        <td colSpan="6" className="px-6 py-8 text-center text-gray-400 dark:text-gray-500">
                                            {activeTab === 'active' ? 'No active admins found.' : 'No deactivated admins found.'}
                                        </td>
                                    </tr>
                                ) : (
                                    displayedAdmins.map((admin) => (
                                        <tr key={admin.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                            <td className="px-6 py-4 text-gray-700 dark:text-gray-300 font-medium">{admin.id}</td>
                                            <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{admin.name}</td>
                                            <td className="px-6 py-4 text-gray-700 dark:text-gray-300">{admin.email}</td>
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                                    admin.is_active === false || admin.is_active === 0
                                                        ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
                                                        : 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400'
                                                }`}>
                                                    {admin.is_active === false || admin.is_active === 0 ? 'Inactive' : 'Active'}
                                                </span>
                                            </td>
            ...
            {showCreateModal && (
                <ModalOverlay onClose={() => setShowCreateModal(false)}>
                    <h2 className="text-lg font-bold text-gray-800 dark:text-gray-200 mb-4">Create New Admin</h2>
            ...
            {showEditModal && selectedAdmin && (
                <ModalOverlay onClose={() => setShowEditModal(false)}>
                    <h2 className="text-lg font-bold text-gray-800 dark:text-gray-200 mb-4">Edit Admin</h2>
                ...
                <ModalOverlay onClose={() => setShowDeactivateModal(false)}>
                    <div className="text-center">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                            This will deactivate <span className="font-semibold text-gray-700 dark:text-gray-300">{selectedAdmin.name}</span>'s account.
                        </p>
            ...
            {showActivateModal && selectedAdmin && (
                <ModalOverlay onClose={() => setShowActivateModal(false)}>
                    <div className="text-center">
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                            This will reactivate <span className="font-semibold text-gray-700 dark:text-gray-300">{selectedAdmin.name}</span>'s account.
                        </p>
                        <div className="flex justify-center gap-3">
